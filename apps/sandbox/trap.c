...
/* ==================================================================================== */
/**
 * Set the callback function used to intercept system calss. boxer_syscall_cb is just an
 * alias for a function pointer. See dune.h
 */
...
/* ==================================================================================== */
/**
 * Use the passed trap frame to transfer to thread.
 */
...
/* ==================================================================================== */
/**
 * Create child thread by ?copying parent's thread information? and jumping
 * to callback function based on trap frame.
 */
...
/* ==================================================================================== */
/**
 * Create new thread, uses initializes mutex and calls callback function pthread_entry(),
 * this is where dune_enter() is called from.
 */
...
static long dune_clone(struct dune_tf *tf)
{
  ...
  /* This is the child. Enter Dune mode as this is not automatically done. */
  if (rc == 0) {
    dune_enter();
    dune_set_user_fs(fs);
  }
...
/* ==================================================================================== */
/**
 * All system call parameters must be checked before they are turned into hypercalls.
 * As the paper states, we have to be able to make sure the the sytem call cannot
 * corrupt of disable our sandbox runtime.

 * It is unclear to me exactly how someone would disable the sandbox but based on then
 * things this switch statement checks for, it appears we're checking for:

 * 1) Out of bounds memory accesses.
 * 2) Malformed strings.
 * 3) Arguments to system calls that the VM uses for manegement? (Not sure about this one).
 */
...
/* ==================================================================================== */
/**
 * Call the callback function that we registered through boxer_register_syscall_monitor(),
 * this function decides the returned value.
 * If not set, error out by with an "operation not allowed".
 */
...
/* ==================================================================================== */
/**
 * Here we actually perform the system call. Some system calls we implement ourselves
 * (such as clone), otherwise the default thing to do it to pass the system call "up"
 * through a hypercall to the VMM.
 */
...
/* ==================================================================================== */
/**
 * Intercepts all system calls. Make sure the parameters "are good" to avoid malicious
 * program from breaking sandbox abstraction. Then go to the user defined filter:
 * sycall_allow(). If both of these return true, perform the system call. Otherwise
 * just return without performing it.
*/
